...
# from config import CONFERENCE_SERVER
#
#
...
#         chat = "%s@%s" % (chatID, CONFERENCE_SERVER)
...
#                 groupchatPresence(chat, self.get_user_data(owner)["name"], vk2xmpp(self.UserID), "unavailable")
...
#                 groupchatPresence(chat, self.get_user_data(usr)["name"], vk2xmpp(usr))
#             groupchatSetConfig(chat, _owner)
#             member(chat, self.jidFrom, _owner)
#             inviteUser(chat, self.jidFrom, _owner, self.get_user_data(owner)[
#                 "name"]) ## TODO: Handle WHO invited me. Yes, i know that it'll be never happen. But maybe someone another do it for himself? You're welcome!
...
#                 uName = self.get_user_data(user)["name"]
#                 user = vk2xmpp(user)
